...
export const getCandyMachineState = async (
  ...
  try {
    ...
    const itemsAvailable = Number(state.data.itemsAvailable);
    const itemsRedeemed = Number(state.itemsRedeemed);
    const itemsRemaining = itemsAvailable - itemsRedeemed;
    const goLiveDate = new Date(Number(state.data.goLiveDate) * 1000);
...
export const awaitTransactionSignatureConfirmation = async (
  ...
  queryStatus = false
): Promise<anchor.web3.SignatureStatus | undefined> => {
  let done = false;
  let status: anchor.web3.SignatureStatus | undefined = {
    slot: 0,
    confirmations: 0,
    err: null,
  };
